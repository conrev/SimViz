...
#define _UNLIT 1
	...
	void Ext_SurfaceFunction0 (inout Surface o, ShaderData d)
	{
		...
		float4 gloss   = SAMPLE_TEXTURE2D(_MetallicGlossMap, sampler_MetallicGlossMap, coord);
		float4 bump    = SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, coord);
		float4 glow    = SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, coord);

		#if !_SKIP_ALPHA_CLIP
			clip(texMain.a * d.vertexColor.a * _Color.a - 0.5f);
		#endif

		o.Albedo     = texMain.rgb * d.vertexColor.rgb * _Color.rgb;
		...
		o.Alpha      = texMain.a * d.vertexColor.a * _Color.a;
	}
	...
	void Ext_ModifyVertex2 (inout VertexData v, inout ExtraV2F e)
	{
		e.extraV2F0.xyz = mul(GetObjectToWorldMatrix(), v.vertex).xyz;
		#if _HDRP
